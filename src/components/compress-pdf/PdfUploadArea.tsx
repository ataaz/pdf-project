...
export function PdfUploadArea({ onFileAdded, className, isProcessing }: PdfUploadAreaProps) {
  ...
  const effectiveClass = cn(
    'flex flex-col items-center justify-center p-8 border-2 border-dashed rounded-lg transition-colors duration-200 ease-in-out h-full',
    isProcessing 
  ...
  return (
    ...
    >
      ...
      <p className={cn("text-lg font-semibold text-center", isDragging ? "text-primary" : "text-foreground")}>
        {isProcessing ? 'Processing...' : 'Drag & drop a PDF file here'}
      </p>
      <p className={cn("text-sm text-center", isDragging ? "text-primary/80" : "text-muted-foreground")}>
